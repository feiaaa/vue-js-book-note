...
```html
// 理解样例
<div v-on:click="saySelf('click from inner')" v-on:click="saySelf('click from self')">
...
别名：enter, tab, delete, esc, space, up, down, lef, right。

使用：`<input v-on:keyup.enter="submit"/>` 

自定义别名：`Vue.config.keyCode.f1=12` 

### 2.5 Vue.extend()

> 生成可被重复使用的子组件，创建基础Vue构造器的子类。

```javascript
var Child = Vue.extend({
    template: '#child',
    // 不同的是，el和data选项需要通过函数返回值，避免多个组件实例共用一个数据
    data: function(){
        return {
            ...
        }
